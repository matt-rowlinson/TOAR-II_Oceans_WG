...
df=pd.DataFrame(df)
df.columns=['Value']
df['2009-07-15' : '2009-09-30'] = np.nan
cv=df
...
for n, df in enumerate(dfs):
    ...
    CV.plot_o3_curve_from_df(df,X, Y, output, timestep=timestep, pref=pref[n], savepath='plots/')
...
plt.legend()
plt.ylabel('$O_3$ (ppb)')
plt.savefig('plots/ozone_trends.png', dpi=200)
plt.close()
